...
CTR=0
	...
	sleep 2
	mkdir at_etx_${VALUE}
	mv COOJA* log*txt at_etx_${VALUE}
	mv *pcap at_etx_${VALUE}
	mv tun*txt at_etx_${VALUE}
	mv nohup.out at_etx_${VALUE}
	mv route.out at_etx_${VALUE}
	mv at_etx_${VALUE} ${BASE_TEST}/.
	cd $BASE_TEST
	mv $LOGFILE at_etx_${VALUE}/.
	mv udp.out at_etx_${VALUE}/.
	mv coordinator.out at_etx_${VALUE}/.
