...
CTR=0
	...
	sleep 2
	mkdir uc_rx_${VALUE}
	mv COOJA* log*txt uc_rx_${VALUE}
	mv *pcap uc_rx_${VALUE}
	mv tun*txt uc_rx_${VALUE}
	mv nohup.out uc_rx_${VALUE}
	mv route.out uc_rx_${VALUE}
	mv uc_rx_${VALUE} ${BASE_TEST}/.
	cd $BASE_TEST
	mv $LOGFILE uc_rx_${VALUE}/.
	mv udp.out uc_rx_${VALUE}/.
	mv coordinator.out uc_rx_${VALUE}/.
