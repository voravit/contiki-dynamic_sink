...
CTR=0
	...
	sleep 2
	mkdir uc_nbr_${VALUE}
	mv COOJA* log*txt uc_nbr_${VALUE}
	mv *pcap uc_nbr_${VALUE}
	mv tun*txt uc_nbr_${VALUE}
	mv nohup.out uc_nbr_${VALUE}
	mv route.out uc_nbr_${VALUE}
	mv uc_nbr_${VALUE} ${BASE_TEST}/.
	cd $BASE_TEST
	mv $LOGFILE uc_nbr_${VALUE}/.
	mv udp.out uc_nbr_${VALUE}/.
	mv coordinator.out uc_nbr_${VALUE}/.
