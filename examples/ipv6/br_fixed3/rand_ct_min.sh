...
CTR=0
	...
	cp -p coordinator.c.min coordinator.c
...
sed -i "s/THRESHOLD_HIT 3/THRESHOLD_HIT ${THRESHOLD}/" /home/voravit/repo/current/main/contiki/core/net/rpl/rpl-metric-timer.c
	...
	sleep 2
	mkdir ct_min_${VALUE}
	mv COOJA* log*txt ct_min_${VALUE}
	mv *pcap ct_min_${VALUE}
	mv tun*txt ct_min_${VALUE}
	mv nohup.out ct_min_${VALUE}
	mv route.out ct_min_${VALUE}
	mv ct_min_${VALUE} ${BASE_TEST}/.
	cd $BASE_TEST
	mv $LOGFILE ct_min_${VALUE}/.
	mv udp.out ct_min_${VALUE}/.
	mv coordinator.out ct_min_${VALUE}/.
