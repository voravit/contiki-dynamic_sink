...
CTR=0
	...
	cp -p coordinator.c.max coordinator.c
...
sed -i "s/THRESHOLD_HIT 3/THRESHOLD_HIT ${THRESHOLD}/" /home/voravit/repo/current/main/contiki/core/net/rpl/rpl-metric-timer.c
	...
	sleep 2
	mkdir ct_max_${VALUE}
	mv COOJA* log*txt ct_max_${VALUE}
	mv *pcap ct_max_${VALUE}
	mv tun*txt ct_max_${VALUE}
	mv nohup.out ct_max_${VALUE}
	mv route.out ct_max_${VALUE}
	mv ct_max_${VALUE} ${BASE_TEST}/.
	cd $BASE_TEST
	mv $LOGFILE ct_max_${VALUE}/.
	mv udp.out ct_max_${VALUE}/.
	mv coordinator.out ct_max_${VALUE}/.
