...
CTR=0
	...
	#sed -i "2156s/<bound>false/<bound>true/" run.csc
        cp -p project-conf.h.exp1 project-conf.h
...
#        cp -a /home/voravit/repo/current/main/contiki/core/net/rpl/rpl-metric-timer.c.rank /home/voravit/repo/current/main/contiki/core/net/rpl/rpl-metric-timer.c
        cp -a /home/voravit/repo/current/main/contiki/core/net/rpl/rpl-metric-timer.c.op234 /home/voravit/repo/current/main/contiki/core/net/rpl/rpl-metric-timer.c
	...
	sleep 2
	mkdir op_234_${RATE}
	mv COOJA* log*txt op_234_${RATE}
	mv *pcap op_234_${RATE}
	mv tun*txt op_234_${RATE}
	mv nohup.out op_234_${RATE}
	mv route.out op_234_${RATE}
	mv op_234_${RATE} ${BASE_TEST}/.
	cd $BASE_TEST
	mv $LOGFILE op_234_${RATE}/.
	mv udp.out op_234_${RATE}/.
	mv coordinator.out op_234_${RATE}/.
